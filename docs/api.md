...
- Chatbot (AI relay)
  - `POST /api/chatbot/chatbot/`  -- send a user message to the server which forwards to OpenAI
    - Body (JSON): `{ "message": "Hello, tell me about X" }`
    - Response (JSON): `{ "reply": "...assistant reply..." }`
    - Notes: server requires `OPENAI_API_KEY` environment variable or Django setting. The server will return 503 if key is missing.
...
If you want, I can:
- Save an extracted `swagger.json` locally (call the running server's `/swagger.json`) — requires server running.
